...
export async function POST(req: Request) {
  try {
    ...
    const statsContext = stats ? `
      ...
      - البلاغات المحلولة: ${stats.resolved}
      - البلاغات التي لم تُحل: ${stats.unresolved}
      - بلاغات أخرى معلقة: ${stats.other}
      - نسبة الإنجاز: ${stats.resolvedPercentage}%
    ` : 'لا توجد إحصائيات متوفرة حالياً.';

    const systemPrompt = `
      أنت "مساعد بلدي الذكي" (Baladi AI Assistant).
